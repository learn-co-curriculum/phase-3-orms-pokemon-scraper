...
describe "Pokemon" do
  ...
  describe "has caught all 151 from scraping" do
    it "has all 151 pokemon" do
      expect(@db.execute("has_all_pokemon").flatten.first).to eq(151)
    end

    it "knows all the information about Horsea" do
      expect(@db.execute("knows_horsea").flatten).to eq([116, "Horsea", "Water"])
    end

    it "knows Psyduck is the 54th pokemon" do
      expect(@db.execute("knows_psyduck").flatten.first).to eq(54)
    end

    it "knows pokemon with the id 143 is Snorlax" do
      expect(@db.execute("knows_snorlax").flatten.first).to eq("Snorlax")
    end

    it "knows Charmander's type is fire" do
      expect(@db.execute("knows_charmander").flatten.first).to eq("Fire")
    end
  end
  ...
  describe "saving more pokemon to the database" do
    ...
    it "knows the pokemon count increases" do
      expect(@db.execute("knows_count_inrease").flatten.first).to eq(152)
    end

    it "has Togepi as the last pokemon" do
      expect(@db.execute("knows_togepi_is_last").flatten.first).to eq("Togepi")
    end
  end
